  ...
  return (
    <AppShell title="Team" actions={actions || undefined}>
      <div className="p-lg max-w-4xl">
        <div className="flex gap-xs border-b border-outline-variant mb-lg overflow-x-auto flex-nowrap scrollbar-none">
          {TABS.map((t) => (
            <button
              key={t.key}
              onClick={() => setTab(t.key)}
              className={`flex items-center gap-xs px-md py-2 text-body-md border-b-2 -mb-px transition-colors shrink-0 whitespace-nowrap ${
                tab === t.key
...
function InvitesCard({ wsId }: { wsId: string }) {
  console.log("🔵 InvitesCard nhận wsId =", wsId); 
