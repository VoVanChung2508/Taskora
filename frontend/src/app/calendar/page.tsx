...
function getHanoiNow(): Date {
  // Lấy giờ hiện tại theo múi giờ Asia/Ho_Chi_Minh (UTC+7), bất kể máy người dùng đặt giờ gì
  const now = new Date();
  const utcMs = now.getTime() + now.getTimezoneOffset() * 60000; // quy về UTC
  const hanoiMs = utcMs + 7 * 60 * 60000; // cộng thêm 7 tiếng
  return new Date(hanoiMs);
}
...
function NowLine() {
  ...
  const min = (currentHour - START_HOUR) * 60 + currentMin;
  const top = (min / 60) * 80; // 80 = chiều cao mỗi ô giờ (ROW_H)

  return (
    <div className="absolute left-0 right-0 z-20 pointer-events-none" style={{ top }}>
      <div className="h-[2px] bg-red-500 relative">
        <span className="absolute -left-1 -top-1 w-2.5 h-2.5 rounded-full bg-red-500" />
      </div>
    </div>
  );
}
