...
export function StatTile({
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-lg md:rounded-2xl p-3 md:p-5 shadow-sm hover:shadow-md transition-shadow">
      <div className="flex items-start justify-between mb-2 md:mb-4">
        <p className="text-xs md:text-[15px] font-semibold text-gray-700">{title}</p>
        <Icon name="more_horiz" size={16} className="text-gray-300 md:text-lg" />
      </div>
      <div className="flex items-end justify-between gap-2 md:gap-3">
        <div className="min-w-0">
          <p className="text-lg md:text-[30px] leading-none font-bold text-gray-900 tracking-tight">{value}</p>
          {delta !== undefined && (
            <p className={`flex items-center gap-1 mt-1.5 md:mt-3 text-xs md:text-[13px] font-semibold ${good ? "text-green-600" : "text-red-500"}`}>
              <Icon name={up ? "trending_up" : "trending_down"} size={14} className="md:w-4 md:h-4" />
              {up ? "+" : ""}{delta.toFixed(2)}%
            </p>
          )}
        </div>
...
export function TrendAreaChart({
  ...
  const H = width < 425 ? 250 : height;
  const padL = width < 425 ? 38 : 46;
  const padR = width < 425 ? 8 : 12;
  const padT = 12;
  const padB = width < 425 ? 28 : 34;
  ...
  return (
    <div className="relative w-full" ref={wrapRef}>
      <svg viewBox={`0 0 ${W} ${H}`} width="100%" height={H} style={{ display: "block" }}>
        ...
        {[0, 1, 2, 3, 4].map((i) => {
          ...
          return (
            <g key={i}>
              ...
              <text x={padL - 10} y={y + 4} textAnchor="end" fontSize={width < 425 ? 11 : 13} fill="#9aa2ad" fontWeight={600}>
                {formatCompact(v)}
              </text>
            </g>
          );
        ...
        {labels.map((l, i) => {
          const every = Math.max(1, Math.ceil(labels.length / Math.max(2, Math.floor(innerW / (width < 425 ? 50 : 70)))));
          const keep = i === 0 || i === labels.length - 1 || i % every === 0;
          if (!keep) return null;
          // Nudge the edge labels inward so they aren't clipped by the viewBox.
          const anchor = i === 0 ? "start" : i === labels.length - 1 ? "end" : "middle";
          return (
            <text key={i} x={xAt(i)} y={H - 10} textAnchor={anchor} fontSize={width < 425 ? 10 : 12} fill="#6b7280" fontWeight={600}>
              {l}
            </text>
          );
        })}
      ...
      {hover !== null && (
        <div
          className="absolute pointer-events-none bg-gray-900 text-white rounded-xl px-2 md:px-3 py-1.5 md:py-2 shadow-lg text-xs md:text-[13px] z-10"
          style={{
      ...
      {/* legend */}
      <div className="flex flex-wrap items-center justify-center gap-3 md:gap-6 mt-2 text-xs md:text-[13px]">
        {series.map((s) => (
          <span key={s.key} className="flex items-center gap-2 text-gray-600">
