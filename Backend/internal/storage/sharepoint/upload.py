...
# Cấu trúc thư mục con chuẩn mà Flowie tạo bên dưới mỗi thư mục project, để
# các file được tổ chức nhất quán.
...
# Kích thước mà trên đó Graph yêu cầu dùng upload session. Microsoft ghi
# nhận 4 MB là giới hạn trần cho một PUT thông thường.
SIMPLE_UPLOAD_LIMIT = 4 << 20

# chunk_size phải là bội số của 320 KiB theo hợp đồng của Graph API.
_CHUNK_SIZE = 5 * 320 * 1024  # 1.6 MiB


@dataclass(frozen=True)
class ChunkRange:
    """Mô tả một lát cắt của file cần gửi đi."""

    start: int
    end: int  # bao gồm cả điểm cuối, theo yêu cầu của Content-Range


def _plan_chunks(size: int, chunk: int = 0) -> List[ChunkRange]:
    """Chia kích thước file thành các lát Content-Range.

    Được tách riêng khỏi phần code mạng để phần số học — chỗ dễ sai lệch
    một byte nhất — có thể được unit test mà không cần gọi Graph.
    """
    if size <= 0:
        return []
    if chunk <= 0:
    ...
    while start < size:
        ...
        out.append(ChunkRange(start=start, end=end))
        start += chunk
    return out
...
class Client:
    ...
    # ---- Đường dẫn thư mục theo Workspace / Project / Task -----------

    def workspace_folder(self, workspace_slug: str) -> str:
        """Trả về đường dẫn thư mục (tương đối trong drive) của một
        workspace bên dưới thư mục gốc đã cấu hình, ví dụ
        "Flowie/acme-corp"."""
        return posixpath.normpath(
            posixpath.join(self.root_folder, workspace_slug)
        )

    def project_folder(self, workspace_slug: str, project_slug: str) -> str:
        """Trả về đường dẫn thư mục (tương đối trong drive) của một
        project, ví dụ "Flowie/acme-corp/MKT-website-revamp"."""
        return posixpath.normpath(
            posixpath.join(self.root_folder, workspace_slug, project_slug)
        )

    def task_folder(
        self, workspace_slug: str, project_slug: str, task_ref: str
    ) -> str:
        """Trả về đường dẫn thư mục (tương đối trong drive) dùng để lưu file
        đính kèm của một task, nằm lồng bên trong thư mục 04_Tasks của
        project."""
        return posixpath.normpath(
            posixpath.join(
                self.root_folder, workspace_slug, project_slug, "04_Tasks", task_ref
            )
        )