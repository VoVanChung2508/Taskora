...
func developmentWorkspaceName(user *domain.User) string {
	if user == nil {
		return "Development Workspace"
	}
	if strings.TrimSpace(user.DisplayName) != "" {
		return fmt.Sprintf("%s Workspace", strings.TrimSpace(user.DisplayName))
	}
	if strings.TrimSpace(user.Email) != "" {
		return fmt.Sprintf("%s Workspace", strings.TrimSpace(user.Email))
	}
	return "Development Workspace"
}

func (h *Handlers) ensureDevelopmentWorkspace(ctx context.Context, user *domain.User) (*domain.Workspace, error) {
	if user == nil {
		return nil, nil
	}
	items, err := h.Store.Workspaces.ListForUser(ctx, user.ID)
	if err != nil {
		return nil, err
	}
	if len(items) > 0 {
	...
	name := developmentWorkspaceName(user)
	slug := util.Slugify(name)
	ws, err := h.Store.Workspaces.Create(ctx, name, slug, user.ID)
	if err != nil {
		return nil, err
	}
	return ws, nil
...
// Thêm GET /dev/login để bỏ qua Azure.
...
// 		↓
// 	Cập nhật.
// Như vậy developer không cần Azure Account.
	user, err := h.Store.Users.UpsertFromAzure(r.Context(), "dev|"+email, email, name, "", isAdmin)
	if err != nil {
		httpx.Error(w, http.StatusInternalServerError, "provision_failed", err.Error())
		return
	}
	// Quy trình đăng nhập dành cho nhà phát triển cũng hỗ trợ MFA,
	// nhờ đó có thể kiểm thử luồng xác thực ngay trên môi trường cục bộ
	// và endpoint này sẽ không bao giờ trở thành kẽ hở để vượt qua 2FA.
	...
	if h.Cfg.Env == "development" {
		if _, err := h.ensureDevelopmentWorkspace(r.Context(), user); err != nil {
			slog.Error("ensure development workspace", "user", user.ID, "error", err)
		}
	}
